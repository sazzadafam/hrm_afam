...
# ==========================================================
# 1. STATIC ROUTES (No parameters, match these first)
# ==========================================================
...
def get_all_employees(db: Session = Depends(get_db), current_user = Depends(roles_required(["admin", "read_only_admin"]))):
    ...
    for user in users:
        ...
        results.append({
            ...
            "monthly_paid_leave": p.monthly_paid_leave if p else 0,
            "shift_end": p.shift_end if p else "20:00",            
            "has_split_shift": p.has_split_shift if p else False,            
            "image_path": p.image_path if p else None,
        })
    return results

@router.get("/archived")
def get_archived_employees(db: Session = Depends(get_db)): 
    archived = (
        db.query(Employee)
...
async def create_employee_and_user(
    ...
    new_employee = Employee(
        ...
        shift_end=shift_end,
        duty_hour=duty_hour,
        has_split_shift=True if str(has_split_shift).lower() == "true" else False,
        salary=salary,
...
def delete_employee_permanently(user_id: int, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
...
async def update_employee(
    ...
    employee.monthly_paid_leave = monthly_paid_leave    
    employee.has_split_shift = True if str(has_split_shift).lower() == "true" else False    
...
def get_employee_details(
    ...
    return {
        ...
        "has_split_shift": profile.has_split_shift,        
        "duty_hour": profile.duty_hour,
        "monthly_paid_leave": profile.monthly_paid_leave,
...
#     # --- FIX: Catch the boolean string from React FormData ---
#     has_split_shift: str = Form("false"),
...
#         # --- FIX: Convert string to Python Boolean ---
#         has_split_shift=True if str(has_split_shift).lower() == "true" else False,
...
#     # Return the full profile data (don't use the shortened version you had at the bottom)
#     return {
#         "id": user.id,
#         "name": user.name,
#         "email": user.email,
...
#     # SQLAlchemy will handle cascading deletes if your models are configured,
#     # otherwise delete the employee profile first then the user.
#     db.delete(user)
#     db.commit()
#     return {"status": "success", "message": "Record permanently purged"}